...
static std::string decode(String const& encoded_string, bool remove_linebreaks)
{
	...
	while (pos < length_of_string)
	{
		...
		//
		ret.push_back(
			static_cast<std::string::value_type>(((pos_of_char(encoded_string.at(pos + 0))) << 2) + ((pos_of_char_1 &
				0x30) >> 4)));
			...
			)
		{
				...
				)
			{
				...
				//
				ret.push_back(
					static_cast<std::string::value_type>(((pos_of_char_2 & 0x03) << 6) + pos_of_char(
						encoded_string.at(pos + 3))));
			}
		}
